...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> { const { locale } = await params; if (!isLocale(locale) || locale === "en") return {}; const t = copy(locale); return { title: { absolute: `NoFi Diary — ${t.heroTitle}` }, description: t.heroLead, alternates: { canonical: localePath(locale), languages: { en: "/", it: "/it", fr: "/fr", es: "/es", de: "/de" } } }; }
