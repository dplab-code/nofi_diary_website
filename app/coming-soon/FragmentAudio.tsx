...
export function FragmentAudio({ src, duration, labels, fragment, gain = 1, bars = 22 }: { src: string; duration: number; labels: { play: string; pause: string; timeline: string }; fragment: string; gain?: number; bars?: number }) {
  ...
  return <div className={`${styles.audio} ${playing ? styles.playing : ""}`}>
    ...
    <div className={styles.wave} aria-hidden="true">{Array.from({ length: bars }, (_, index) => <i key={index} />)}</div>
