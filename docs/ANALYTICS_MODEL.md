...
Vercel Web Analytics is configured in `app/layout.tsx`. Store CTAs use `TrackedStoreLink`; the real-memory gallery records only the first audio start per fragment and page mount. Event properties are deliberately low-cardinality and must never contain personal data or diary content.
...
Out of scope: scroll depth, heatmaps, mouse movement, session recording, fingerprinting, persistent IDs, cross-session profiles, per-element clicks and app analytics.
