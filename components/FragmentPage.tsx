...
export function FragmentPage({ fragment, locale }: { fragment: NofiFragment; locale: Locale }) {
  ...
  return <div className={styles.page}><ComingSoonHeader locale={locale} currentPath={`/fragments/${fragment.slug}`} /><FragmentVisit fragment={fragment.id} locale={locale} /><main className={styles.main}><Link className={styles.back} href={home}>← {backCopy[locale]}</Link><article className={styles.article}><p className={styles.eyebrow}>NoFi Diary · Fragment {fragment.id}</p><h1>{copy.title}</h1><p className={styles.excerpt}>{copy.excerpt}</p><div className={styles.memoryFrame}><div className={styles.memory}><Image src={fragment.image} alt={copy.imageAlt} fill priority sizes="(max-width: 700px) 92vw, 860px" /></div></div>{fragment.audio && <section className={styles.audioCard} aria-label={pageCopy.voiceLabel}><p>{pageCopy.voiceLabel}</p><FragmentAudio src={fragment.audio} duration={23} gain={2.25} labels={{ play: pageCopy.play, pause: pageCopy.pause, timeline: pageCopy.timeline }} fragment={fragment.id} /></section>}<p className={styles.body}>{copy.body}</p><Link className={`${styles.back} ${styles.bottomBack}`} href={home}>← {backCopy[locale]}</Link></article></main><footer className={styles.footer}><strong>NoFi</strong><p>{pageCopy.footerLine}</p><Link className={styles.privacy} href={localePath(locale,"/privacy")}>{privacyCopy[locale]}</Link><span>© 2026</span></footer></div>;
}
