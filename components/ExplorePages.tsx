...
function Stickers({ locale }: { locale:Locale }) { const t=exploreCopy(locale),p=t.stickers,ui=exploreUi[locale],examples=stickerExamples[locale]; return <>
  ...
  <section className="appEvidence appEvidenceReverse stickerIntro section shell"><div><p className="kicker">{ui.expression}</p><h2>{p.vocabularyTitle}</h2><p>{p.vocabularyBody}</p></div><div className="androidScreen"><Image src="/images/app/stickers-nofi.png" alt="A curated NoFi sticker library beside a personal memory preview" fill sizes="(max-width:700px) 78vw, 30vw" /></div></section>
...
function Appearances({ locale }: { locale:Locale }) { const t=exploreCopy(locale),p=t.appearances,ui=exploreUi[locale],panel=appearanceDifferenceCopy[locale]; return <>
  ...
  <section className="appearanceDifference section"><div className="shell differenceBook">
    <article className="differencePanel differenceTheme"><Link className="differencePanelLink" href={localePath(locale,"/themes")} aria-label={`${panel.explore}: ${p.theme}`}/><div className="differenceVisual" aria-hidden="true"><span className="paperLayer paperLayerBack"/><span className="paperLayer paperLayerMiddle"/><span className="memorySheet"><i/><i/><i/></span></div><div className="differenceContent"><p className="kicker">01 / 02</p><h2>{p.theme}</h2><p className="differenceDescription">{panel.theme}</p><ul>{p.themeItems.map((x,i)=><li className={i===0?"isActive":undefined} key={x}>{i===0&&<span/>}{x}</li>)}</ul></div><span className="differenceExplore">{panel.explore}<b aria-hidden="true">→</b></span></article>
    <article className="differencePanel differenceAppearance"><Link className="differencePanelLink" href={`${localePath(locale,"/appearances")}#appearance-editions`} aria-label={`${panel.explore}: ${p.appearance}`}/><div className="differenceVisual" aria-hidden="true"><span className="appGlow"/><span className="appShell"><i/><i/><i/><b/><em/></span></div><div className="differenceContent"><p className="kicker">02 / 02</p><h2>{p.appearance}</h2><p className="differenceDescription">{panel.appearance}</p><ul>{p.appearanceItems.map((x,i)=><li className={i===1?"isActive":undefined} key={x}>{i===1&&<span/>}{x}</li>)}</ul></div><span className="differenceExplore">{panel.explore}<b aria-hidden="true">→</b></span></article>
  </div></section>
