...
function Themes({ locale }: { locale:Locale }) { const t=exploreCopy(locale),p=t.themes,ui=exploreUi[locale]; return <>
  ...
  <section className="themeComparison section shell"><article><div className="phoneCrop productScreenshot"><Image src="/images/app/theme-original.jpeg" alt="Original Paper theme shown in NoFi Diary on Android" fill sizes="(max-width:700px) 82vw, 32vw"/></div><p className="kicker">{t.common.included}</p><h2>Original Paper</h2><p>{p.originalCopy}</p></article><article><div className="phoneCrop productScreenshot"><Image src="/images/app/theme-old-summer.jpeg" alt="Old Summer Feeling theme shown in NoFi Diary on Android" fill sizes="(max-width:700px) 82vw, 32vw"/></div><p className="kicker">{t.common.coming}</p><h2>Old Summer Feeling</h2><p>{p.summerCopy}</p></article></section>
