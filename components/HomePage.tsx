...
export function HomePage({ locale }: { locale: Locale }) {
  ...
  return <main>
    ...
    <section className="section compareSection"><div className="shell compareGrid"><div><p className="kicker">A different kind of app</p><h2>{t.compareTitle}</h2><div className="comparePhoto"><Image src="/images/editorial/gallery-pages-v2.png" alt="" fill sizes="35vw" /></div></div><div className="compareTable"><div className="compareRow head"><b>{t.typical}</b><b>{t.nofi}</b></div>{t.comparison.map(([a,b]) => <div className="compareRow" key={a}><span>{a}</span><span>{b}</span></div>)}</div></div></section>
